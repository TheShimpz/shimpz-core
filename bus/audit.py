"""Structured audit log for every mutating bus-driver operation.
...
AUDIT_PATH = Path(os.environ.get("SHIMPZ_BUSDRIVER_AUDIT_LOG", "/var/log/bus-driver/audit.jsonl"))
...
def log(
    ...
    `subject` is whatever this op is about (a topic, a project, …) — returns the trace_id
    (generated if not given) for the caller to echo back in its HTTP response.
    """
