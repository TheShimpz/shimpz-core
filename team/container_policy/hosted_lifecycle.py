...
def _put_inbox_file(
    ...
    with _controller._lock_for(team_id):
        ...
        try:
            stored = _controller._storage().put(team_id, filename, data, media_type)
        except team_storage.StorageQuotaError as exc:
            raise _controller.ApiError(HTTPStatus.INSUFFICIENT_STORAGE, str(exc)) from exc
...
def _list_team_files(team_id: str, lease: _controller._AuthorizationLease) -> dict:
    with _controller._lock_for(team_id):
        _controller._require_current_authorization(team_id, lease, require_isolation=False)
        try:
            listing = _controller._storage().list(team_id)
        except team_storage.StorageError as exc:
            raise _controller.ApiError(HTTPStatus.SERVICE_UNAVAILABLE, "Team storage failed its safety checks") from exc
        return {"team_id": team_id, **listing}


def _delete_team_file(team_id: str, file_id: object, lease: _controller._AuthorizationLease) -> dict:
    with _controller._lock_for(team_id):
        _controller._require_current_authorization(team_id, lease, require_isolation=False)
        try:
            result = _controller._storage().delete(team_id, file_id)
        except team_storage.StorageNotFoundError as exc:
            raise _controller.ApiError(HTTPStatus.NOT_FOUND, "file not found") from exc
        except team_storage.StorageError as exc:
            raise _controller.ApiError(HTTPStatus.SERVICE_UNAVAILABLE, "Team storage failed its safety checks") from exc
...
def _remove_volume(team_id: str, kind: str) -> bool:
    ...
    try:
        volume = _controller._docker.volumes.get(name)
        volume.reload()
...
def _owned_teardown_brain(team_id: str, owner: str, brain_id: str):
    try:
        brain = _controller._get_container(manifests.team_container_name(team_id))
    except docker.errors.DockerException:
        return False, None
    if brain is None:
        return True, None
...
def _stop_teardown_brain(brain) -> bool:
    if brain is None:
        return True
    try:
        _controller._fail_stop_team(brain, timeout=30)
...
def _teardown_apps(team_id: str) -> bool:
    try:
        app_containers = _controller._team_app_containers(team_id)
    except docker.errors.DockerException:
        return False
    ...
    for app_container in app_containers:
        ...
        result = _controller._teardown_app(team_id, app_id, container=app_container, drop_db=False)
        cleanup_complete = result.artifacts_removed and cleanup_complete
    return cleanup_complete


def _teardown_network_planes(team_id: str) -> bool:
    return _controller._teardown_team_networks(team_id)


def _remove_teardown_brain(brain) -> bool:
    if brain is None:
        return True
    return _controller._remove_team_container(brain, timeout=30)


def _teardown_volumes(team_id: str) -> bool:
    results = [
        _controller._remove_volume(team_id, kind)
...
def _teardown_storage(team_id: str) -> bool:
    if _controller._storage_instance is None and not _controller.TEAM_STORAGE_ROOT.exists():
        return True
    try:
        _controller._storage().destroy(team_id)
...
def _teardown_inference(team_id: str) -> bool:
    try:
        _controller._inference_store.delete(team_id)
...
def _teardown_assistant_secrets(team_id: str) -> bool:
    ...
    try:
        _controller._assistant_secrets.delete_team(team_id)
...
def _teardown_assistant_accounts(team_id: str) -> bool:
    ...
    try:
        _controller._assistant_accounts.delete_team(team_id)
    except oauth_account_store.OAuthAccountStoreError:
        return False
    return _controller._teardown_team_approval_grants(team_id)
...
def _teardown(team_id: str, *, owner: str, brain_id: str) -> _controller._CleanupResult:
    """Remove every Team artifact, preserving a durable owner-bound retry anchor throughout."""
    brain_valid, brain = _controller._owned_teardown_brain(team_id, owner, brain_id)
    if not brain_valid:
    ...
    try:
        ...
        return _controller._CleanupResult(False, False)
    if (
        not _controller._stop_teardown_brain(brain)
        or not _controller._teardown_apps(team_id)
        or not _controller._teardown_storage(team_id)
        or not _controller._teardown_inference(team_id)
        or not _controller._teardown_assistant_secrets(team_id)
        or not _controller._teardown_assistant_accounts(team_id)
        or not _controller._teardown_network_planes(team_id)
        or not _controller._remove_teardown_brain(brain)
        or not _controller._teardown_volumes(team_id)
    ):
        return _controller._CleanupResult(False, record.db_dropped)
    record = _controller._drop_teardown_database(team_id, record)
    if record is None:
        return _controller._CleanupResult(False, False)
    ...
    if not _controller._finalize_teardown(team_id, record):
        return _controller._CleanupResult(False, True)
    return _controller._CleanupResult(True, True)


def _create(team_id: str, body: dict, owner: str = "") -> dict:
    try:
        team_name = _controller._validated_team_name(body.get("team_name", team_id))
    ...
    with _controller._lock_for(team_id):
        pending_cleanup = _controller._cleanup_record(team_id)
        ...
        existing = _controller._get_container(manifests.team_container_name(team_id))
        if existing is not None:
            ...
            existing_name = _controller._team_name_from_anchor(existing)
            if "team_name" in body and team_name != existing_name:
                raise _controller.ApiError(HTTPStatus.CONFLICT, "Team name differs from the persisted identity")
            _controller._inference_store.save(team_id, inference)
            return {
                "team_id": team_id,
        ...
        if not _controller._teardown_storage(team_id):
            raise _controller.ApiError(
        ...
        with _controller._reserve_capacity(f"team:{team_id}", owner, manifests.MEM_LIMIT_BYTES, team_slot=True):
            ...
            try:
                ...
                _controller._wire_network_deps(network, manifests.core_deps())
...
def _destroy(team_id: str, lease: _controller._AuthorizationLease) -> dict:
    with _controller._lock_for(team_id):
        # Destruction is the supported remediation for a legacy or drifted runtime.
        if lease.cleanup_nonce:
            _controller._require_cleanup_authorization(team_id, lease)
            container = None
        else:
            container = _controller._require_current_authorization(
                team_id,
                lease,
        ...
        chat_lock = _controller._chat_lock_for(team_id)
        if container is not None:
            container.reload()
            if container.status == "running":
                _controller._fail_stop_team(container, timeout=30)
        if not chat_lock.acquire(timeout=30):
            raise _controller.ApiError(HTTPStatus.CONFLICT, "the active chat turn did not stop in time")
        try:
            try:
                _controller._brain_runtime.delete_thread(_controller._brain_thread_id(team_id, lease.container_id))
            ...
            try:
                _controller._power_execution_journal().purge(lease.container_id)
            except power_journal.PowerJournalError as exc:
                raise _controller.ApiError(
                    HTTPStatus.SERVICE_UNAVAILABLE,
                    "Team Power execution state could not be deleted",
                ) from exc
            cleanup = _controller._teardown(team_id, owner=lease.owner, brain_id=lease.container_id)
            _controller._clear_team_id_runtime_state(team_id)
            if not cleanup.complete:
                raise _controller.ApiError(
                    HTTPStatus.INTERNAL_SERVER_ERROR,
...
def _list(owner: str | None = None) -> dict:
    ...
    return {"teams": [_controller._describe(container) for container in teams]}


def _status(team_id: str, lease: _controller._AuthorizationLease) -> dict:
    with _controller._lock_for(team_id):
        # Status remains readable so the UI can offer Stop/Destroy remediation.
        return _controller._describe(
            _controller._require_current_authorization(team_id, lease, require_isolation=False)
        )


def _inference_status(team_id: str, lease: _controller._AuthorizationLease) -> dict:
    with _controller._lock_for(team_id):
        _controller._require_current_authorization(team_id, lease)
        try:
            config = _controller._inference_store.load(team_id)
        except inference_config.InferenceConfigError as exc:
            raise _controller.ApiError(HTTPStatus.CONFLICT, "Team model provider is not configured") from exc
    return {"team_id": team_id, "provider": config.provider, "model": config.model}


def _configure_inference(team_id: str, body: object, lease: _controller._AuthorizationLease) -> dict:
    if not isinstance(body, dict) or set(body) != {"provider", "model"}:
        raise _controller.ApiError(HTTPStatus.UNPROCESSABLE_ENTITY, "inference requires provider and model")
    try:
    ...
    with _controller._lock_for(team_id):
        _controller._require_current_authorization(team_id, lease)
        try:
            _controller._inference_store.save(team_id, config)
        except inference_config.InferenceConfigError as exc:
            raise _controller.ApiError(
...
def _logs(team_id: str, lines: int, lease: _controller._AuthorizationLease) -> dict:
    with _controller._lock_for(team_id):
        container = _controller._require_current_authorization(team_id, lease, require_isolation=False)
        return {"team_id": team_id, "logs": container.logs(tail=lines).decode("utf-8", "replace")}


@_controller._serialize_against_team_chat
def _lifecycle(team_id: str, op: str, lease: _controller._AuthorizationLease) -> dict:
    with _controller._lock_for(team_id):
        ...
        container = _controller._require_current_authorization(team_id, lease, require_isolation=op != "stop")
