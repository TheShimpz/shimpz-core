...
class R2DriverClientTests(unittest.TestCase):
    ...
    def test_principal_is_atomic_owner_only_and_stable_under_concurrency(self) -> None:
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(client, "PRINCIPAL_DIR", Path(directory)):
            ...
            def create() -> None:
                tokens.append(client._principal("team_1", create=True))
            ...
            token_path = Path(directory) / "team_1.token"
    ...
    def test_principal_rejects_symlink_mode_and_hardlink(self) -> None:
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(client, "PRINCIPAL_DIR", Path(directory)):
            ...
            target.chmod(0o600)
            (root / "team_1.token").symlink_to(target)
            with self.assertRaises(client.R2DriverError):
                client._principal("team_1", create=False)

            (root / "team_1.token").unlink()
            target.chmod(0o640)
            os.link(target, root / "team_1.token")
            with self.assertRaises(client.R2DriverError):
                client._principal("team_1", create=False)
    ...
    def test_finalize_removes_principal_only_after_upstream_200(self) -> None:
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(client, "PRINCIPAL_DIR", Path(directory)):
            root = Path(directory)
            root.chmod(0o700)
            client._principal("team_1", create=True)
            path = root / "team_1.token"
            with (
                ...
                client.finalize_team_drop("team_1")
    ...
    def test_retire_is_retryable_without_a_local_principal(self) -> None:
        ...
        with (
            ...
            client.retire_team("team_1")
            client.retire_team("team_1")
        self.assertEqual(len(calls), 2)
        self.assertTrue(all(item[0][1] == "/v1/teams/retire" for item in calls))
        self.assertTrue(all(item[0][2] == {"team_id": "team_1"} for item in calls))
