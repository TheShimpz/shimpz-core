"""Fixed, bounded team-driver client for Team-scoped R2 credentials.

Only this controller persists a cleartext Team principal.  R2 stores its hash and all credential
...
PRINCIPAL_DIR = Path(os.environ.get("SHIMPZ_R2_PRINCIPAL_DIR", "/var/lib/team-driver/r2-principals"))
...
def _team_id(value: object) -> str:
    if not isinstance(value, str) or _TEAM_ID_RE.fullmatch(value) is None:
        raise R2DriverError(HTTPStatus.BAD_REQUEST, "Team id is invalid", category="request")
    return value
...
def _principal_path(team_id: object) -> Path:
    return PRINCIPAL_DIR / f"{_team_id(team_id)}.token"
...
def _principal(team_id: object, *, create: bool) -> str:
...
def _remove_principal(team_id: object) -> None:
    path = _principal_path(team_id)
    with _principal_guard:
        _ensure_principal_directory()
        try:
...
def provision_team(team_id: object) -> dict[str, object]:
...
def ensure_provisioned(team_id: object) -> None:
    # Replaying provision is intentional: it resolves an ambiguous earlier HTTP result without local state.
    provision_team(team_id)


def retire_team(team_id: object) -> dict[str, object]:
    team_id = _team_id(team_id)
    return _call(
        "POST",
        "/v1/teams/retire",
...
def finalize_team_drop(team_id: object) -> dict[str, object]:
    team_id = _team_id(team_id)
    result = _call(
        "POST",
        "/v1/teams/finalize",
...
def driver_document(team_id: object) -> dict[str, object]:
    team_id = _team_id(team_id)
    driver = _project_driver(_call("GET", "/v1/driver"))
    credential_form = _project_form(_call("GET", "/v1/driver/credentials"))
    inventory = _call("GET", f"/v1/teams/{team_id}/credentials", bearer=_principal(team_id, create=False))
...
def create_credential(team_id: object, payload: dict[str, object]) -> dict[str, object]:
    team_id = _team_id(team_id)
    result = _call(
        "POST",
        f"/v1/teams/{team_id}/credentials",
        payload,
        bearer=_principal(team_id, create=False),
    )
    return _project_credential(result)


def rotate_credential(team_id: object, credential_id: object, payload: dict[str, object]) -> dict[str, object]:
    team_id, crid = _team_id(team_id), _credential_id(credential_id)
    result = _call(
        "PUT",
        f"/v1/teams/{team_id}/credentials/{crid}",
        payload,
        bearer=_principal(team_id, create=False),
    )
    return _project_credential(result)
...
def remove_credential(team_id: object, credential_id: object, payload: dict[str, object]) -> dict[str, object]:
    team_id, crid = _team_id(team_id), _credential_id(credential_id)
    result = _call(
        "DELETE",
        f"/v1/teams/{team_id}/credentials/{crid}",
...
def verify_credential(team_id: object, credential_id: object) -> dict[str, object]:
    team_id, crid = _team_id(team_id), _credential_id(credential_id)
    result = _call(
        "POST",
        f"/v1/teams/{team_id}/credentials/{crid}/verify",
        {},
        bearer=_principal(team_id, create=False),
    )
