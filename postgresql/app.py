...
def _create_app(body: dict, token: str) -> dict:
    ...
    with postgresql_client.mutation_lock():
        ...
        database = postgresql_client.dbname(project)
        if database not in scoped and postgresql_client.project_resources_exist(project):
            raise principal_store.PrincipalError("unregistered App database artifacts already exist")
        result = postgresql_client.create_db_and_role(project, allow_existing=database in scoped)
        try:
...
def _drop_app(body: dict, token: str) -> dict:
    ...
    with postgresql_client.mutation_lock():
        ...
        if database not in scoped:
            ...
            if postgresql_client.project_resources_exist(project):
        ...
        result = postgresql_client.drop_db_and_role(project)
