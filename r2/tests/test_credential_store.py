...
def bundle(*, bucket: str = "team-bucket", secret: str = "b" * 64) -> dict[str, str]:
    return {
        "account_id": "a" * 32,
...
class CredentialStoreTests(unittest.TestCase):
    ...
    def test_create_is_idempotent_private_and_metadata_only(self) -> None:
        ...
        self.assertEqual(self.store.list_metadata("team_one"), (first,))
        self.assertEqual(
            set(first.public()),
            {
                "team_id",
        ...
        self.assertNotIn(bundle()["secret_access_key"], repr(self.store.resolve("team_one", "primary-r2")))
    ...
    def test_create_scopes_hmac_idempotency_and_rejects_changed_payload(self) -> None:
        ...
        self.assertEqual(second.team_id, "team_two")
        state = self.state()["teams"]
        first_hash = state["team_one"]["primary-r2"]["idempotency_hash"]
        second_hash = state["team_two"]["secondary-r2"]["idempotency_hash"]
    ...
    def test_rotation_uses_new_nonce_and_compare_and_swap(self) -> None:
        self.create()
        old_nonce = self.state()["teams"]["team_one"]["primary-r2"]["envelope"]["nonce"]
        ...
        new_nonce = self.state()["teams"]["team_one"]["primary-r2"]["envelope"]["nonce"]

        self.assertEqual(rotated.generation, 2)
        self.assertEqual(rotated.label, "Rotated R2")
        self.assertNotEqual(old_nonce, new_nonce)
        self.assertEqual(self.store.resolve("team_one", "primary-r2").value("secret_access_key"), "c" * 64)
        with self.assertRaises(CredentialConflictError):
            self.store.rotate(
                "team_one",
                "primary-r2",
        ...
        with self.assertRaises(CredentialValidationError):
            self.store.rotate("team_one", "primary-r2", 2, "other", "Invalid", bundle())

    def test_aad_prevents_envelope_swaps_and_cross_team_access(self) -> None:
        self.create()
        self.create(
            team_id="team_two",
            credential_id="secondary-r2",
        ...
        with self.assertRaises(CredentialNotFoundError):
            self.store.resolve("team_two", "primary-r2")

        state = self.state()
        first = state["teams"]["team_one"]["primary-r2"]
        second = state["teams"]["team_two"]["secondary-r2"]
        ...
        with self.assertRaises(CredentialStoreError):
            self.store.resolve("team_one", "primary-r2")
        with self.assertRaises(CredentialStoreError):
            self.store.resolve("team_two", "secondary-r2")

    def test_remove_destroys_envelope_and_is_retry_idempotent(self) -> None:
        self.create()
        removed = self.store.remove("team_one", "primary-r2", 1)
        retried = self.store.remove("team_one", "primary-r2", 1)

        self.assertEqual(removed, retried)
        self.assertEqual(removed.generation, 2)
        self.assertEqual(removed.status, "revoked")
        self.assertIsNone(self.state()["teams"]["team_one"]["primary-r2"]["envelope"])
        self.assertEqual(self.store.list_metadata("team_one"), ())
        self.assertEqual(self.store.team_record_count("team_one"), 1)
        with self.assertRaises(CredentialRevokedError):
            self.store.resolve("team_one", "primary-r2")
        self.assertEqual(self.store.remove("team_one", "primary-r2", 2), removed)
        self.store.purge_revoked("team_one", "primary-r2", 2)
        with self.assertRaises(CredentialNotFoundError):
            self.store.resolve("team_one", "primary-r2")
    ...
    def test_concurrent_idempotent_create_commits_once(self) -> None:
        ...
        self.assertEqual(len(self.state()["teams"]["team_one"]), 1)
    ...
    def test_health_authenticates_state_and_requires_its_key(self) -> None:
        ...
        removed = empty.remove("team_one", metadata.credential_id, 1)
        empty.purge_revoked("team_one", metadata.credential_id, removed.generation)
