...
def values(*, bucket: str = "team-bucket", secret: str = "d" * 64) -> dict[str, str]:
    return {
        "account_id": "e" * 32,
...
def create_body(idempotency_key: str, *, bucket: str = "team-bucket", secret: str = "d" * 64):
    return {
...
class AppLifecycleTests(unittest.TestCase):
    ...
    def provision(self, team_id: str, principal_token: str) -> None:
        status, payload = self.request(
            "POST",
            "/v1/teams/provision",
            {"team_id": team_id, "principal_token": principal_token},
    ...
    def test_many_credentials_are_isolated_atomic_and_never_publicly_secret(self) -> None:
        status, _ = self.request(
            "POST",
            "/v1/teams/provision",
            {"team_id": "team_a", "principal_token": TEAM_A_TOKEN},
            token=MANAGED_TOKEN,
        )
        self.assertEqual(status, 403, "the managed data-plane token must not provision Teams")
        self.provision("team_a", TEAM_A_TOKEN)
        self.provision("team_b", TEAM_B_TOKEN)
        status, _ = self.request("GET", "/v1/teams/team_a/credentials")
        self.assertEqual(status, 403)

        first_body = create_body("11111111-1111-4111-8111-111111111111")
        status, first = self.request("POST", "/v1/teams/team_a/credentials", first_body, token=TEAM_A_TOKEN)
        ...
        status, second = self.request(
            "POST",
            "/v1/teams/team_a/credentials",
            create_body("22222222-2222-4222-8222-222222222222", bucket="team-a-two"),
            token=TEAM_A_TOKEN,
        )
        self.assertEqual(status, 200)
        status, third = self.request(
            "POST",
            "/v1/teams/team_b/credentials",
            create_body("11111111-1111-4111-8111-111111111111", bucket="team-b-one"),
            token=TEAM_B_TOKEN,
        )
        self.assertEqual(status, 200)
        self.assertEqual(len({first["id"], second["id"], third["id"]}), 3)

        status, listed_a = self.request("GET", "/v1/teams/team_a/credentials", token=TEAM_A_TOKEN)
        self.assertEqual(status, 200)
        self.assertEqual({item["id"] for item in listed_a["credentials"]}, {first["id"], second["id"]})
        status, _ = self.request("GET", "/v1/teams/team_b/credentials", token=TEAM_A_TOKEN)
        self.assertEqual(status, 404, "a valid bearer for another Team must not disclose scope existence")
        ...
        status, removed = self.request(
            "DELETE",
            f"/v1/teams/team_a/credentials/{first['id']}",
        ...
        for secret in (*values().values(), "f" * 64, first_body["idempotency_key"], TEAM_A_TOKEN):
    ...
    def test_provider_failure_does_not_persist_and_teardown_is_retry_safe(self) -> None:
        ...
        self.assertEqual(app.credential_store.STORE.list_metadata("team_b"), ())
        self.probe_error = None
        ...
        status, removed = self.request(
            "DELETE",
            f"/v1/teams/team_b/credentials/{created['id']}",
            {"expected_generation": 1},
            token=TEAM_B_TOKEN,
        )
        ...
        status, _ = self.request("POST", "/v1/teams/finalize", {"team_id": "team_b"}, token=PROVISIONER_TOKEN)
        self.assertEqual(status, 409)
        ...
        replay_status, _ = self.request(
            "POST",
            "/v1/teams/provision",
            {"team_id": "team_b", "principal_token": TEAM_B_TOKEN},
            token=PROVISIONER_TOKEN,
        )
        self.assertEqual(replay_status, 409)
        self.provision("team_b", "3" * 64)
    ...
    def test_retire_waits_for_in_flight_probe_then_revokes_the_result(self) -> None:
        ...
        state = json.loads(app.credential_store.STORE.state_path.read_text())
        self.assertIsNone(state["teams"]["team_a"][created_id]["envelope"])
        status, _ = self.request("GET", "/v1/teams/team_a/credentials", token=TEAM_A_TOKEN)
        self.assertEqual(status, 404)
