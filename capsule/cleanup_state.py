"""Durable, bounded authorization state for retrying an incomplete Capsule destroy.
...
STATE_DIR = Path(os.environ.get("SHIMPZ_CAPSULE_CLEANUP_DIR", "/var/lib/capsule-driver/cleanup"))
MAX_RECORDS = int(os.environ.get("SHIMPZ_CAPSULE_CLEANUP_MAX_RECORDS", "128"))
MAX_RECORD_BYTES = 4096
VERSION = 1

_CID_RE = re.compile(r"^[a-z0-9_]{1,40}$")
_BRAIN_ID_RE = re.compile(r"^(?:[a-f0-9]{12,64})?$")
_NONCE_RE = re.compile(r"^[a-f0-9]{32}$")
_guard = threading.RLock()

if MAX_RECORDS < 1:
    raise ValueError("SHIMPZ_CAPSULE_CLEANUP_MAX_RECORDS must be positive")
...
def _validate_record(record: Record) -> Record:
    if (
        ...
        or not isinstance(record.cid, str)
        or _CID_RE.fullmatch(record.cid) is None
    ):
...
def _path(cid: str) -> Path:
    if _CID_RE.fullmatch(cid) is None:
        raise CleanupStateError("invalid Capsule id for cleanup state")
    return STATE_DIR / f"{cid}.json"
...
def _load_unlocked(cid: str) -> Record | None:
    path = _path(cid)
    try:
...
def load(cid: str) -> Record | None:
    with _guard:
        _ensure_directory()
        return _load_unlocked(cid)
...
def _write_unlocked(record: Record) -> None:
    ...
    temporary = STATE_DIR / f".{record.cid}.{secrets.token_hex(8)}.tmp"
    descriptor = -1
    try:
        descriptor = os.open(
            temporary,
        ...
        os.close(descriptor)
        descriptor = -1
        temporary.replace(_path(record.cid))
        _fsync_directory()
    except OSError as exc:
...
def begin(cid: str, owner: str, brain_id: str) -> Record:
    ...
    with _guard:
        _ensure_directory()
        existing = _load_unlocked(cid)
        if existing is not None:
            if (existing.owner, existing.brain_id) != (owner, brain_id):
                raise CleanupStateError("cleanup record identity does not match this Capsule")
            return existing
        try:
...
def mark_db_dropped(record: Record) -> Record:
    ...
    with _guard:
        current = _load_unlocked(record.cid)
